...
import IngredientToCartIcon from "../_components/add-ingredient-to-cart-icon";
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="flex justify-between border-b p-4">
        ...
        <AllIngredientsToCart recipeId={ingredients[0].recipeId} ingredientIds={ingredientIds} isActive />
      </div>
      <ul>
        {ingredients.map(({ title, id, recipeId }) => (
          <li key={id} className="flex justify-between border-b px-4 py-2">
            <p>{title}</p>
            <IngredientToCartIcon id={id} recipeId={recipeId} isActive />
          </li>
        ))}
      </ul>
