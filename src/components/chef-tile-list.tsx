...
const ChefTileList = async ({ chefs }: Props) => {
  return (
    <ul className="px-4">
      {chefs.map(async ({ id, name, profileImage, profile, _count }) => (
          ...
          <ChefTile id={id}>
            <div className="flex gap-x-4">
              ...
              <div className="flex flex-col items-start self-center">
                <p className="line-clamp-1 text-xl font-bold text-mauve12">{name}</p>
                <p className="mt-2 line-clamp-3 text-mauve11">{profile}</p>
                <div className="mt-1 flex items-center gap-2">
                  ...
                  <span className="text-mauve11">{_count.Recipe} レシピ</span>
                </div>
              </div>
            </div>
          </ChefTile>
